function hh = clabel_logx(cs, varargin)
    ...
    %   $Revision: 5.38.4.25 $  $Date: 2011/09/23 19:12:04 $
    ...
    % Modified further by theethan to account for log-scaled axes 2015/07/06
    % theethan, 2015
    ...
    if ishg2parent( varargin{:} )       
        if nout == 0
            clabelHGUsingMATLABClasses(cs, varargin{:});
        else % nout == 1
            hh = clabelHGUsingMATLABClasses(cs, varargin{:});
        end
    else
        error(nargchk(1, Inf, nargin, 'struct'));
        if min(size(cs)) > 2
            error(message('MATLAB:clabel:InvalidContourMatrixInput'));
        end
        ...
        if nargin == 1
            ...
            if firstArgValidObj
                if false ... % <------------------------------------------- HACK: just skip this, i.e. don't use the 'innate' specgraph.contourgroup ShowText property
                   && isa(handle(varargin{1}),'specgraph.contourgroup') ...
                ...
                else % <--------------------------------------------------- HACK: always do this, i.e. use the inline_labels function, since that's what I've modified
...
function H = inline_labels(cax, CS, h, varargin)
    ...
    % [ethan] could check the axes scales here, e.g.
    % [ethan]   strncmpi(get(cax,'XScale'),'log',3)
    % [ethan] etc.
    ...
    xyc = inf(2,10); % [ethan] keep track of centre points
    lxyc = size(xyc,2); ixyc = 0; % [ethan] note length & last-used idx
    ii = 1;
    k = 0;
    while (ii < lCS)
        if manual
        ...
        if ~manual % [ethan] move the ii update here so that if no points
            ii = ii + 1 + CS(2, ii); % [ethan] to add later, can continue
        end % [ethan]
...
%             % [ethan] -->
%             % Shift the points around a little to accomodate log scale?
%             xl = x(Il); xc = x(Ic); xr = x(Ir); dxl = xc-xl; dxr = xr-xc;
%             
%             % [ethan] <--
...
%                 fprintf('\n\n'); % [ethan] DEBUG
%                 fprintf('Aspx: [%.2f]->[%.2f]\n',Aspx,PA(3)/log(XL(2)/XL(1))); % [ethan] DEBUG
%                 fprintf('Aspy: [%.2f]->[%.2f]\n',Aspy,PA(3)/log(YL(2)/YL(1))); % [ethan] DEBUG
%                 for i=1:length(xl) % [ethan] DEBUG
%                     fprintf('\n\n'); % [ethan] DEBUG
%                     fprintf('x:    [%.2f,%.2f]->[%.2f,%.2f]\n',xl(i),xr(i),log10(xl(i)),log10(xr(i))); % [ethan] DEBUG
%                     fprintf('y:    [%.2f,%.2f]->[%.2f,%.2f]\n',yl(i),yr(i),log10(yl(i)),log10(yr(i))); % [ethan] DEBUG
%                 end; clear i; % [ethan] DEBUG
                ...
                % [ethan] -->
                % Identify the points too close to those already labelled
                % Dimension during calculation: xy - old pts. - new pts.
                rx = 1.2; ry = 0.1; % radii of exclusion for labels
%                 dxyc = bsxfun(@plus, ... % figure the distance
%                               permute([xc;yc],[1 3 2]),-xyc);
%                 dxyc = bsxfun(@times,dxyc,... % rescale to figure units
%                               1./[diff(XL);diff(YL)*PA(3)/PA(4)]);
                dxyc = bsxfun(@plus, ... % figure the distance
                              permute([log10(xc);yc],[1 3 2]),... % log x
                              -[log10(xyc(1,:));xyc(2,:)]);
                dxyc = bsxfun(@times,dxyc,... % rescale to figure units
                              1./[log10(XL(2)/XL(1));diff(YL)*PA(3)/PA(4)]); % log x
                qs = permute( sind(-trot), [1 3 2]); % rotation bases
                qc = permute( cosd(-trot), [1 3 2]);
                ec = ( bsxfun(@times,qc,dxyc(1,:,:)) ... % elliptical
                       - bsxfun(@times,qs,dxyc(2,:,:)) ).^2/rx^2 ... % zone
                     + ( bsxfun(@times,qs,dxyc(1,:,:)) ... % of exclusion
                         + bsxfun(@times,qc,dxyc(2,:,:)) ).^2/ry^2;
                ec(isnan(ec)) = Inf; % restore inf in case of inf-inf
                qq = squeeze(min(ec,[],2)) > 0.01; % not too-close points
                lp = sum(qq); % trim the too-close points out
                xl = xl(qq); xr = xr(qq); xc = xc(qq); 
                yl = yl(qq); yr = yr(qq); yc = yc(qq);
                Il = Il(qq); Ir = Ir(qq); %Ic = Ic(qq);
                trot = trot(qq); psn = psn(qq); psnI = psnI(qq);
                qq = qq(qq); % [ethan] DEBUG
                % [ethan] <--
                
                % [ethan] Add the new points to the list of label points
                if isempty(xc), continue; end;
                while ixyc+lp>lxyc, % [ethan] expand if necessary
                    xyc = [xyc zeros(2,lxyc)]; lxyc = 2*lxyc; % [ethan] 2x
                end % [ethan]
                xyc(:,ixyc+(1:lp)) = [xc;yc]; % [ethan] add
                ixyc = ixyc+lp; % [ethan] update index
                ...
                dr = d(Ir) + sqrt((log10(xr./x(Ir)) * Aspx) .^ 2 + ((yr - y(Ir)) * Aspy) .^ 2); % [ethan]
                dl = d(Il) + sqrt((log10(xl./x(Il)) * Aspx) .^ 2 + ((yl - y(Il)) * Aspy) .^ 2); % [ethan]
                ...
                for jj = 1 : lp
                    ...
                    if ~isempty(z)
                        ...
                        if ~qq(jj), set(H(end),'Color',[1 0 0]); end % [ethan] DEBUG
                    else
