function [odata,b] = interp_fix(idata,r,l,cutoff)
...
%   $Revision: 1.7.4.7 $  $Date: 2010/05/20 03:09:34 $
...
if (2*l+1 > size(idata,1) && size(idata,1) > 1) ...
        || (2*l+1 > size(idata,2) && size(idata,1) == 1)
	s = int2str(2*l+1);
	error(generatemsgid('InvalidDimensions'),...
        'Length of data sequence must be at least %s.\n You either need more data or a shorter filter (L).',s);
end
...
[mo,no] = size(idata);
if no>1 && mo==1 % in case of row vector
    idata = idata.';
end
[m,n] = size(idata);
odata = zeros(r*m,n);
...
od = zeros(2*l*r,n);
od(1:r:(2*l)*r,:) = 2*idata(m,:)-(idata((m-1):-1:(m-2*l),:));
od = filter(b,1,od,zf);
odata(m*r-l*r+1:m*r, :) = od(1:l*r, :);
